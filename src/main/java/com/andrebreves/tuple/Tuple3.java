...
public final class Tuple3<T1, T2, T3> implements Tuple, Comparable<Tuple3<T1, T2, T3>> {
    ...
    public <R> Tuple3<R, T2, T3> mapV1(Function<T1, R> mapper) {
        return Tuple3.of(mapper.apply(v1), v2, v3);
    }

    /** Returns a Tuple mapping the 2nd value using the giving mapper function, and keeping the remaining values unchanged. */
    public <R> Tuple3<T1, R, T3> mapV2(Function<T2, R> mapper) {
        return Tuple3.of(v1, mapper.apply(v2), v3);
    }

    /** Returns a Tuple mapping the 3rd value using the giving mapper function, and keeping the remaining values unchanged. */
    public <R> Tuple3<T1, T2, R> mapV3(Function<T3, R> mapper) {
        return Tuple3.of(v1, v2, mapper.apply(v3));
    }

    @FunctionalInterface
    public interface ValuesConsumer<T1, T2, T3> {
        void accept(T1 v1, T2 v2, T3 v3);
    }

    /** Consumes the values of this Tuple using the giving Consumer. */
    public void consumeValues(ValuesConsumer<T1, T2, T3> consumer) {
        consumer.accept(v1, v2, v3);
    }

    @FunctionalInterface
    public interface ValuesFunction<T1, T2, T3, R> {
        R apply(T1 v1, T2 v2, T3 v3);
    }

    /** Maps the values of this Tuple using the giving Function. */
    public <R> R mapValues(ValuesFunction<T1, T2, T3, R> function) {
        return function.apply(v1, v2, v3);
    }

    @FunctionalInterface
    public interface ValuesPredicate<T1, T2, T3> {
        boolean test(T1 v1, T2 v2, T3 v3);
    }

    /** Test the values of this Tuple using the giving Predicate. */
    public boolean testValues(ValuesPredicate<T1, T2, T3> predicate) {
        return predicate.test(v1, v2, v3);
    }

    /** Returns a Tuple containing the values of this Tuple and the values passed as parameters. */
    public <T4> Tuple4<T1, T2, T3, T4> append(T4 v4) {
    ...
    public Tuple3<T1, T2, T3> concat(Tuple0 t) {
        return this;
    }
